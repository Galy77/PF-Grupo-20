...
const Cart = () => {
  ...
  const getCart = async () => {
    const { data } = await axios(
      `https://api-market-henry-jczt.onrender.com/pf/cart/${usuarioActual.id}`
    );
    if (data.Products.length) {
      let allPrice = data.Products.map((el) => el.price);
      allPrice = Math.round(
        allPrice.reduce(function (acc, numero) {
          return acc + numero;
        }, 0)
      );
      setTotal(allPrice);
    ...
    <>
      <div class="mt-4">
        ...
        <div class="cart-container d-flex justify-content-evenly">
          <div
            id="cart-card"
            class="d-flex flex-column align-items-center justify-content-center cuerpo"
          >
            <CartCart
              products={products}
          ...
          <div id="aux">
            <div class="total-container d-flex flex-column justify-content-evenly align-items-center">
              <span id="total" className="txt-large">
                TOTAL
              </span>
              <span class="txt-large">{`$${total}`}</span>

              <button onClick={handleBuy} id="pay">
                Pay
              </button>
