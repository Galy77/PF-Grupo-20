...
const Details = () => {
  ...
  const getReviews = async () => {
    const { data } = await axios(
      `https://api-market-henry-jczt.onrender.com/pf/review/${id}`
    );
  ...
  if (!isBuy) {
    if (user) {
      axios
        .get(`https://api-market-henry-jczt.onrender.com/pf/payment/${user.id}`)
        .then((response) => {
  ...
  if (!orders.length) {
    if (productosStorage !== null)
      productosStorage.map((el) => dispatch(addOrder(el)));
  }
  ...
  const addCart = async () => {
    ...
    if (cart.data.response == "no hay carrito") {
      ...
      try {
        if (data)
          await axios.post(
            "https://api-market-henry-jczt.onrender.com/pf/cart",
            data
          );

        Swal.fire("Producto añadido correctamente al carrito!", "", "success");
      } catch (error) {}
    } else {
      const data = {
  ...
  const createPreference = async () => {
    try {
      const response = await axios.post(
        "https://api-market-henry-jczt.onrender.com/pf/create_preference",
        {
  ...
  const getProductInfo = () => {
    const data = {
      id: product.id,
      cantidad: cantidadProducts
        ? product.stock - cantidadProducts
        : product.stock - 1,
      amount: cantidadProducts
        ? cantidadProducts * product.price
        : product.price,
    };
  ...
  const handleReviews = async () => {
    if (!stars || !coment) alert("completa la review");
      ...
      try {
        await axios.post(
          "https://api-market-henry-jczt.onrender.com/pf/review",
          data
        );
        ...
        if (!review.length) {
          ...
          const response = await axios.put(
            `https://api-market-henry-jczt.onrender.com/pf/rating/${product.id}`,
            data
          );
          dispatch(getProductById(id));
        } else {
          ...
          data =
            data.reduce(function (acc, numero) {
              return acc + numero;
            }, 0) / data.length;
          data = data.toFixed(2);
          data = {
              ...
              <div class="w-100 mt-2 d-flex flex-column justify-content-between align-items-center h-50">
                ...
                >
                  <div class="w-0">
                    <input
                      type="number"
                      placeholder={product.stock ? `stock:${product.stock}` : 0}
                      class="text-center w-100 rounded border"
                      value={cantidadProducts}
                ...
                <div
                  id="btn"
                  class="d-flex w-100 justify-content-end align-items-center mt-4"
                >
                  <button
                    type="button"
                    id="buy"
                ...
                {preferenceId && (
                  <Wallet
                    ...
                    onSubmit={getProductInfo}
                    class="wallet"
                  />
                )}
                {preferenceId && (
                  <p onClick={() => setPreferenceId(null)} class="cancelar">
                    cancelar
                  </p>
                )}
                <div class="w-100 my-5">
                  <h4 class="text-start mt-4 description">
                    {product.description}
                  </h4>
                </div>
              </div>
              ...
              <div class="d-flex flex-column align-items-start h-50 p-4">
                {review ? (
                  review.length ? (
                    ...
                    <span class="text-center w-100">No hay comentarios</span>
                  )
                ) : (
                  ""
                )}
                  ...
                  user ? (
                    review.filter((el) => el.id_user === user.id).length ? (
                      <span class="text-center w-100">
                        No puedes realizar mas reviews!
                      </span>
                    ) : isBuy ? (
                      isBuy.length ? (
                        <div class="h-100 d-flex align-items-center rounded w-100">
                          <div class="d-flex align-items-center justify-content-center w-50 h-100">
                            <div class="">
                              {stars ? (
                                ...
                                <div>
                                  <i
                                    class="stars cursor bi bi-star"
                                    onClick={() => handleStars(0.9)}
                                  ></i>
                                  <i
                                    class="stars cursor bi bi-star"
                                    onClick={() => handleStars(1.9)}
                                  ></i>
                                  <i
                                    class="stars cursor bi bi-star"
                                    onClick={() => handleStars(2.9)}
                                  ></i>
                                  <i
                                    class="stars cursor bi bi-star"
                                    onClick={() => handleStars(3.9)}
                                  ></i>
                                  <i
                                    class="stars cursor bi bi-star"
                                    onClick={() => handleStars(4.9)}
                                  ></i>
                                </div>
                              )}
                              <button
                          ...
                          <div class="d-flex flex-column align-items-center">
                            <input
                              ...
                              class="ml-4 mt-2 text-start w-100"
                              value={coment}
                              onChange={() => handleComent(event)}
                            ></input>
                    ...
                    <span class="text-center w-100">
                      Inicia sesión y compra este producto para hacer un
                      comentario
                    </span>
                  )
                ) : (
