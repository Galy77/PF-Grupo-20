...
function ProductsCards(props) {
  ...
  const sliceProducts = (categoryProducts, page) => {
    ...
    setDataProducts(
      categoryProducts.slice(numToSlice - productsQuantityToShow, numToSlice)
    );
  };
      ...
      <div class="w-100 g-4-products mx-4">
        {dataProducts ? (
          ...
          <h1 className={style.notFound}>
            En esta categoria aun no hay productos
          </h1>
        )}
      </div>
      <div class="d-flex align-items-center">
        {page > 1 ? (
          <p class="m-4 border btn btn-dark" onClick={handlePage}>
            Anterior
          </p>
        ) : (
          ""
        )}
        {lastPage > 1 ? <p class="p">{`${page} de ${lastPage}`}</p> : ""}
        {page < lastPage ? (
          <p class="m-4 border btn btn-dark" onClick={() => handlePage("next")}>
            Siguiente
          </p>
        ) : (
          ""
