...
function Products() {
  ...
  const alphabeticOrderFilter = (productos, orden, sortByPrice) => {
    ...
    if (sortByPrice === "Menor") {
      productos.sort(
        (productoA, productoB) => productoA.price - productoB.price
      );
    } else if (sortByPrice === "Mayor") {
      productos.sort(
        (productoA, productoB) => productoB.price - productoA.price
      );
    }
  ...
  const filteredsProducts = categoryProducts.filter((product) => {
    ...
    const matchRating =
      ...
      (ratingFilterValue === "betterQualified" && product.rating >= 3);
    return matchSearch && matchPrice && matchRating;
  });
  const filteredProducts = alphabeticOrderFilter(
    filteredsProducts,
    alphabeticFilterValue,
    priceOrderValue
  );
    ...
    <div class="d-flex flex-column">
      <div class="filter-products d-flex my-4">
        ...
        <div class="d-flex flex-column align-items-center w-100 px-4">
          <ProductsCards
            categoryProducts={alphabeticOrderFilter(
              categoryProducts,
              alphabeticFilterValue,
              priceOrderValue
            )}
